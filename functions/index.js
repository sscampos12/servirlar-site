...
exports.createStripeCheckout = functions.https.onRequest((req, res) => {
  cors(req, res, async () => {
    ...
    const { servicoId, couponCode, uid, email } = req.body;
    ...
    try {
        ...
        const valorTaxa = valorServico * 0.25;
            ...
            line_items: [
              {
                price_data: {
                    ...
                    unit_amount: Math.round(valorTaxa * 100),
                },
                quantity: 1,
              },
            ],
        ...
        res.status(500).json({ error: { message: error.message || "Não foi possível criar a sessão de pagamento." } });
    }
  });
});
